...
close all;
clear all;
DIR='D:\数学建模竞赛D题附件\附件4-存放结果\静态背景\office\'; 
...
for n = 8:(filenum-2)              %there will be false route line,so it only include the pictures which has the car

    fname = strcat(DIR, num2str(n), '.jpg');
    frame = imread(fname);
    fr = imread(fname);       % read in frame
