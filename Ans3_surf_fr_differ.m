...
SE1 = strel('ball',1,0,0);
SE2 = strel('ball',10,0,0);
SE3 = strel('ball',10,0,0);
...
%    title('Matched inlier points');
    ...
    [ssim_num,~,~,~]=SSIM(rgb2gray(bg),rgb2gray(im));
    fr_diff = abs(double(rgb2gray(im)) - double(rgb2gray(bg)));  % cast operands as double to avoid negative overflow
    for j=1:FRAME_WIDTH                 % if fr_diff > thresh pixel in foreground
        for k=1:FRAME_HEIGHT
...
%    diff_map = vibe.GetfrImageC3R(uint8(bg));
    ...
    diff_map=imerode(diff_map,SE3);
    %diff_map=imdilate(diff_map,SE3);
    bg=im;
    fr_map = immultiply((uint8(diff_map)./255),rgb2gray(bg));
    figure(1)
